defmodule RTP.MessageProcessor do
  use GenServer
  @max_children 10
  ...
  def consume(data) do
    ...
    RTP.MessageProcessorWorker.process(counter, data)
  end
  ...
  defp choose_worker(current_load) do
    ...
    cond do
      ...
      active > current_load or active > 2 ->
        pid = GenServer.whereis(RTP.MessageProcessorWorker.via_tuple(counter))
        case is_pid(pid) do
  ...
  defp worker_spec(id) do
    default_spec = {RTP.MessageProcessorWorker, id}
